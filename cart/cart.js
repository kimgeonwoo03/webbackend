...
module.exports = ({ pool, authMiddleware }) => {
    ...
    try {
        ...
        const processedRows = rows.map(item => {
            let discount = item.discount_rate || 0;

            if (item.sale_start_date && item.sale_end_date) {
                const now = new Date();
                const start = new Date(item.sale_start_date);
                const end = new Date(item.sale_end_date);

                if (!(now >= start && now <= end)) {
                    discount = 0; // ✅ 세일 기간 아님 → 할인 제거
                }
            } else {
                discount = 0; // ✅ 세일 기간 없음 → 할인 없음
            }
        ...
        // ✅ 총 금액 계산
        const totalPrice = processedRows.reduce(
    ...
    try {
        // ✅ 장바구니 + 옵션 + variant + product 가격 정보 조회
        const [cartItem] = await pool.query(`
            SELECT 
                c.cart_id,
                c.option_id,
        ...
        // ✅ 세일 기간 체크 후 할인율 적용
        const { discount, finalPrice } = applySale(
            item.base_price,
            item.discount_rate,
